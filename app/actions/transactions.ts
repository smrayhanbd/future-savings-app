...
// NOTE: `generateMoneyReceiptPdf` is intentionally NOT statically imported here.
// Pulling it in at module load drags `pdfkit` -> `fontkit` -> `@swc/helpers`
// into the same server chunk as `createTransaction` / `submitTransaction`,
// and Turbopack mis-bundles `@swc/helpers`'s `applyDecoratedDescriptor` so it
// becomes `undefined` at runtime — which crashes every deposit save with
// `TypeError: h.applyDecoratedDescriptor is not a function`.
//
// We lazy-load it via dynamic `import()` inside `notifyMember` instead, so
// the pdfkit chain is only evaluated when a receipt actually has to be
// generated (i.e. after an approval). Combined with `serverExternalPackages`
// in next.config.ts, this keeps the broken SWC helper path out of the
// deposit/withdrawal save flow entirely.
type GenerateMoneyReceiptPdf = (payload: Awaited<ReturnType<typeof buildReceiptPayload>>) => Promise<Buffer>
import {
  loadApprovalLimits,
...
async function notifyMember(
  ...
  // 4. Email — richer HTML body, with a money-receipt PDF attached for
  //    deposit/withdrawal approvals. PDF generation is best-effort: if it
  //    fails we still send the notification email without the attachment so
  //    the member is never left uninformed.
  if (member.email) {
    ...
    if (canAttachReceipt) {
      try {
        const payload = await buildReceiptPayload(txn.id)
        if (payload) {
          // Lazy-load pdfkit ONLY when we actually have a receipt to render.
          // See the note above `GenerateMoneyReceiptPdf` for the full reason.
          const { generateMoneyReceiptPdf } = (await import("@/lib/pdf/moneyReceiptPdf")) as {
            generateMoneyReceiptPdf: GenerateMoneyReceiptPdf
          }
          const pdf = await generateMoneyReceiptPdf(payload)
          attachments = [
            {
              filename: `money-receipt-${txn.voucherNo}.pdf`,
              content: pdf,
        ...
        // Log + continue without the attachment.
        console.error("[notifyMember] money receipt PDF failed:", pdfError)
        await prisma.notification.create({
          data: {
            type: "EMAIL_ERROR",
            title: "Money Receipt PDF Failed",
            message: `Failed to generate money receipt PDF for ${txn.voucherNo} (${member.fullName}). Email sent without attachment. Reason: ${(pdfError instanceof Error ? pdfError.message : "") || "Unknown error"}`,
          },
        })
      }
    }
    ...
    try {
      await sendEmail(
        ...
        `
          ...
          <p>${messageBody}</p>
          ${attachments ? `<p style="margin-top:8px">Your money receipt is attached to this email for your records.</p>` : ""}
