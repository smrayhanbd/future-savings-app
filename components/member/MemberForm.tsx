...
type Nominee = {
    ...
    // Nominee signature — required if the nominee is an adult (18+).
...
export default function MemberForm({ mode, member }: MemberFormProps) {
    ...
    const steps = useMemo(() => [
        ...
        { name: "Nominees", complete: formData.nominees.length > 0 && formData.nominees.every(n => !!(n.signatureFile || n.existingSignatureUrl)) },
    ...
    const addNominee = () => {
        ...
        // Nominee signature required if adult (DOB + 18y). Unknown DOB → adult.
        const isAdult = isNomineeAdult(undefined) // nomineeForm has no DOB field; default adult
        if (isAdult && !nomineeForm.signatureFile) {
            toast.error("Nominee signature required", { description: "Adult nominees must provide a signature. Please upload it." })
            return
        }
    ...
    const updateNominee = () => {
        ...
        const isAdult = isNomineeAdult(undefined)
        if (isAdult && !nomineeForm.signatureFile && !nomineeForm.existingSignatureUrl) {
            toast.error("Nominee signature required", { description: "Adult nominees must provide a signature. Please upload it." })
            return
        }
            ...
            <Dialog open={showNomineeModal} onOpenChange={(o) => { setShowNomineeModal(o); if (!o) resetNomineeForm() }}>
                <DialogContent className="max-w-md">
                    ...
                    <div className="space-y-3">
                        ...
                        <div>
                            <Label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Nominee Signature <span className="text-red-500">*</span></Label>
                            {renderFileUpload(
